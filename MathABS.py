...
class ABS:
    ...
    def generateattributes(self, ask, attriblist, attributes):
        '''
        returns signing key SKa
        '''
        ska = {}

        attribute_number = self.createAttributeNumber(attributes)
        ...
        for i in attriblist:
            number = attribute_number[i]
            ska['K{}'.format(number)] = Kbase ** (1 / (ask['a'] + number * ask['b']))

        return ska

    def sign(self, pk, ska, message, policy, attributes): #pk = (tpk,apk)
        '''
        return signature
        '''
        tpk,apk = pk
        ...
        M,u = self.getMSP(policy, attribute_number)
        ...
        for i in range(1,len(M)+1):
            ...
            try: #this fills in for the v vector
                end = multi * (ska['K{}'.format(attribute_number[u[i-1]])] ** r[0])
            except KeyError:
                end = multi
        ...
        for j in range(1,len(M[0])+1):
            ...
            for i in range(1,len(M)+1):
                base = apk['A{}'.format(j)] * (apk['B{}'.format(j)] ** attribute_number[u[i-1]])
                exp = M[i-1][j-1] * r[i]
                end = end * (base ** exp)
    ...
    def verify(self, pk, sign, message, policy, attributes):
        '''
        return bool
        '''
        tpk,apk = pk
        ...
        if sign['Y']==0 or pair(sign['Y'],tpk['h0']) != pair(sign['W'],apk['A0']):
            ...
            for j in range(1,len(M[0])+1):
                ...
                for i in range(1,len(M)+1):
                    a = sign['S{}'.format(i)]
                    b = (apk['A{}'.format(j)] * (apk['B{}'.format(j)] ** attribute_number[u[i-1]])) ** M[i-1][j-1]
    ...
    def createAttributeNumber(self, attributes):
        '''
        属性集合の数字を生成
        先頭から順に2,3,....とする
        '''
...
if __name__ == "__main__":
    ...
    ska = absinst.generateattributes(ask,['SKILLFUL'],attributes)
    lam = absinst.sign((tpk,apk), ska, 'rar', 'SKILLFUL OR ECCENTRIC',attributes)
    print(absinst.verify((tpk,apk),lam,'rar','SKILLFUL OR ECCENTRIC',attributes))
    ska2 = absinst.generateattributes(ask,['SKILLFUL','ECCENTRIC'],attributes)
    lam2 = absinst.sign((tpk,apk), ska2, 'rar', 'SKILLFUL OR ECCENTRIC',attributes)
    print(absinst.verify((tpk,apk),lam2,'rar','SKILLFUL OR ECCENTRIC',attributes))